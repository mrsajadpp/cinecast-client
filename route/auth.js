...
router.post('/verify-otp', async (req, res) => {
  try {
    ...
    const otpRecord = await Code.findOne({ email: email, verification_code: otp }).lean();
    ...
    if (otpRecord.verification_code === parseInt(otp)) {
      const pendingUser = await UserPending.findOne({ email: email }).lean();
      ...
      await UserPending.deleteOne({ email: email });
